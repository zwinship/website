...
both of which are filtered for the selected country. The website allows the user to see visuals like line plots, pie charts, and lat_long maps to help aid their
...
# Calling the image on the website based on the video from Prof. Frydenburg
...
# These columns were spelled wrong for some reason
...
# Originally it wasn't working because as seen above from the data.info(), these columns were int64
...
# Because data_yield has an upper and lower bound I take the average of the two as a new column and then take the average of country input into the function
...
# Calling the image in my folder based on the video from Prof. Frydenburg
img = Image.open("bikini_atoll_test_photo_2.jpg")

# Putting the image on the website based on the video from Prof. Frydenburg
st.image(img, width = 1000)
st.title("Nuclear Explosion Data Analysis")
st.title("CS 230-2")
...
st.write("Kilotons used by deaults if unchecked")
...
# Initializing the subset data for if the date_input is left blank
data_subset = data
...
if st.checkbox("Select Date Range for Observations"):
...
st.write("If you would like to include only observations within a date range, check the box and choose from above. By default the calculation and plot will use all observations")

# Widget to display the average explosion energy for the selected country, uses the above python function get_country_average
...
if st.button("Plot Nuclear Explosion Energy for Selected Country and Time"):
    data_subset = data[(data['date'] >= pd.to_datetime(date_input[0])) & (data['date'] <= pd.to_datetime(date_input[1]))]
    ...
    plt.title('Nuclear Explosion Energy over Chosen Time')
    plt.xlabel('Date')
    plt.ylabel('Nuclear Explosion Energy (Kilotons)')
    st.pyplot(plt)



st.header("Number of Explosion Types by Country")
...
test_type_country = st.selectbox("Select a Country for Explosion Type Analysis", countries)
...
# Widget to show the explosion type data for the selected country and types
if st.button("Show Explosion Type Data"):
    st.write("Number of Explosions by Explosion Type for", test_type_country)
    ...
    if len(keys_input) == 0:
        ...
        plt.ylabel("Number of Explosion")
        st.pyplot(plt)
    else:
...
st.header("Map of All Nuclear Explosion Locations")
...
st.write("Each dot represents the location of Nuclear Explosion, hover over a point to see the country that detonated the bomb and the kilotons of said bomb")
...
st.header("Heat Map of Explosion Energy")
...
st.pydeck_chart(chart)
st.write("Heatmap indicates the magnitude of energy released for bombs within a location, for example, by looking at the heatmap you can easily tell where the highest energy bombs were tested in Russia")
st.write("Zoom in to see more precise magnitudes of the bombs detonated")
